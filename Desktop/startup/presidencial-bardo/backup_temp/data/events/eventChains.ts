	...
	choices: [
		{
			...
			triggeredEvents: ["caos-unificacion"]
		},
		{
...
export const CAOS_UNIFICACION: PoliticalEvent = {
	id: "caos-unificacion",
	title: "💥 Caos por Unificación Cambiaria",
	description: "La unificación de dólares fue un desastre total. Los arbolitos se rebelaron, los importadores no saben qué hacer y los exportadores están perdiendo plata. El mercado está en pánico y el peso se devaluó un 200% en una semana.",
	type: EventType.EMERGENCIA,
	category: EventCategory.ECONOMICO,
	urgency: 5,
	timeLimit: 30,
	trigger: {
		requiredChoices: [
			{ eventId: "guerra-de-dolares", choiceId: "unificar-dolares" }
		],
		probability: 0.8
	},
	choices: [
		{
			id: "volver-multiples-dolares",
			text: "🔄 Volver a múltiples dólares",
			description: "Admitir el error y volver al sistema de múltiples dólares. Es humillante pero necesario.",
			effects: [
				createImmediateEffect(MetricType.ECONOMIA, -10, "Pérdida de credibilidad"),
				createImmediateEffect(MetricType.POPULARIDAD, -20, "Imagen de incompetente"),
				createImmediateEffect(MetricType.CORRUPCION, 10, "Oportunidades de arbitraje restauradas")
			],
			triggeredEvents: ["guerra-de-dolares-v2"]
		},
		{
			id: "dolarizar-economia",
			text: "🇺🇸 Dolarizar la economía",
			description: "Tirar la toalla y adoptar el dólar americano. Es la solución más drástica pero efectiva.",
			effects: [
				createImmediateEffect(MetricType.ECONOMIA, 30, "Estabilidad instantánea"),
				createImmediateEffect(MetricType.POPULARIDAD, -30, "Humillación nacional"),
				createImmediateEffect(MetricType.RELACIONES_INTERNACIONALES, 20, "Responsabilidad económica"),
				createImmediateEffect(MetricType.CORRUPCION, -20, "Menos oportunidades de corrupción cambiaria")
			]
		}
	],
	defaultChoice: "volver-multiples-dolares",
	icon: "💥",
	sound: "caos-economico"
};

export const MERCADO_NEGRO_DOLARES: PoliticalEvent = {
	id: "mercado-negro-dolares",
	title: "🕵️ Mercado Negro de Dólares",
	description: "Con tantos tipos de dólar, el mercado negro floreció como nunca. Los arbolitos de Florida se convirtieron en millonarios, los bancos no saben qué hacer y el gobierno perdió el control total del mercado cambiario.",
	...
	trigger: {
		requiredChoices: [
			{ eventId: "guerra-de-dolares", choiceId: "crear-mas-dolares" }
		],
		probability: 1
	},
	choices: [
		{
			id: "perseguir-arbolitos",
			text: "🚔 Perseguir a los arbolitos",
			description: "Mandar a la AFIP y la policía a perseguir a los arbolitos. Es una batalla perdida pero hay que intentar.",
			effects: [
				createImmediateEffect(MetricType.SEGURIDAD, -10, "Violencia en las calles"),
				createImmediateEffect(MetricType.ECONOMIA, -5, "Mercado paralelo se fortalece"),
				createImmediateEffect(MetricType.POPULARIDAD, -5, "La gente necesita dólares")
			],
			triggeredEvents: ["arbolitos-rebeldes"]
		},
		{
			id: "legalizar-mercado-negro",
			text: "📋 Legalizar el mercado negro",
			description: "Admitir la derrota y legalizar el mercado negro. Es pragmático pero humillante.",
			effects: [
				createImmediateEffect(MetricType.ECONOMIA, 10, "Estabilidad del mercado"),
				createImmediateEffect(MetricType.POPULARIDAD, -10, "Imagen de derrota"),
				createImmediateEffect(MetricType.CORRUPCION, 15, "Legalización de actividades turbias")
			]
		}
	],
	defaultChoice: "perseguir-arbolitos",
	icon: "🕵️",
	sound: "mercado-negro"
};
